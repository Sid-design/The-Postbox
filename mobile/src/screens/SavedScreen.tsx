...
import { SavedScreenProps, Message } from '../navigation/types';
import { apiClient, debugAuth } from '../api/client';
import { useAuth } from '../context/AuthContext';
import { getSenders, Sender } from '../api/client';
...
const Item = ({ message, onPress, onOpenMoreOptions, onToggleRead, onToggleSave, onDelete, selectionMode, selected, onToggleSelect, queuedAction, queuedState, onEnterSelection, summaryMode }: ItemProps & { onOpenMoreOptions: () => void; onToggleRead: () => void; onToggleSave: () => void; onDelete: () => void; selectionMode: boolean; selected: boolean; onToggleSelect: () => void; queuedAction: 'read' | 'save' | 'delete' | null; queuedState: 'on' | 'off'; onEnterSelection: () => void; summaryMode: boolean; }) => {
  ...
  const bgColor = stringToColor(message.sender_name);
    ...
    <Swipeable renderLeftActions={renderLeftActions} renderRightActions={renderRightActions} enabled={!selectionMode && !summaryMode}>
      ...
      >
        <View style={[
          ...
          {/* Content */}
          <View style={{ flex: 1 }}>
            <Text style={[styles.sender, { color: themeColors.text }]}>{message.sender_name}</Text>
            <Text style={[styles.title, { color: themeColors.text }]} numberOfLines={3}>{message.subject}</Text>
            {dateLabel ? (
...
const SavedScreen = ({ navigation }: SavedScreenProps) => {
  ...
  const [contextMenu, setContextMenu] = useState<{ id: number | null; visible: boolean }>({ id: null, visible: false });
  ...
  const unreadCount = useMemo(() => savedMessages.filter(m => !m.is_read).length, [savedMessages]);
  ...
  const filteredMessages = useMemo(() => {
    ...
    return base;
  }, [savedMessages, searchText, selectedGroupId, groups]);
  ...
  // Lazy-load full bodies for summary mode (current and next)
  const fetchMessageBody = useCallback(async (id: number) => {
    try {
      const existing = messages.find(m => m.id === id)?.body_html;
      if (existing) return;
      const resp = await apiClient.get<Message>(`/api/messages/${id}`);
      const body = resp.data.body_html;
      if (body) {
        setMessages(curr => curr.map(m => (m.id === id ? { ...m, body_html: body } : m)));
      }
    } catch {}
  }, [messages, setMessages]);

  const prefetchBodies = useCallback((ids: Array<number | undefined>) => {
    ids.forEach((id) => {
      if (typeof id === 'number') {
        fetchMessageBody(id);
      }
    ...
    navigation.setOptions({
        ...
        selectionMode ? (
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <TouchableOpacity
              onPress={() => {
                if (summaryMode) {
                  // In summary mode, use the currently visible message
                  const currentMessage = filteredMessages[visibleIndex];
                  if (currentMessage) {
                    setContextMenu({ id: currentMessage.id, visible: true });
                  }
                } else {
      ...
      headerRight: () => (
        selectionMode ? (
          ...
          // Summary mode toggle - DISABLED FOR NOW, WILL BE ENABLED IN FUTURE
          // <TouchableOpacity
          //   onPress={() => setSummaryMode(prev => !prev)}
          //   style={{ paddingHorizontal: 12 }}
          //   accessibilityLabel="Toggle summary mode"
          //   activeOpacity={0.8}
          // >
          //   <View style={[styles.summaryToggle, summaryMode ? styles.summaryToggleOn : styles.summaryToggleOff]}>
          //     <View style={[styles.summaryToggleKnob, summaryMode ? { right: 2 } : { left: 2 }]}>
          //       {summaryMode ? (
          //         <MaterialCommunityIcons name="view-agenda" size={14} color={'#6B7280'} />
          //       ) : (
          //         <MaterialCommunityIcons name="view-list" size={14} color={'#6B7280'} />
          //       )}
          //     </View>
          //   </View>
          // </TouchableOpacity>
  ...
  // Update title based on mode: Saved (N) or Select (N)
  useLayoutEffect(() => {
    if (selectionMode) {
      navigation.setOptions({ title: `Select (${selectedIds.size})` });
    } else {
      navigation.setOptions({ title: `Saved (${filteredMessages.length})` });
    }
  ...
  const scrollToTop = () => {
    try {
      if (summaryMode) {
        // In summary mode, use FlatList scrollToOffset
      ...
      // Fallback to scrollToOffset for both
      if (summaryMode) {
        flatListRef.current?.scrollToOffset?.({ offset: 0, animated: true });
      } else {
  ...
  const loadMessages = useCallback(async () => {
    ...
    setError(null);
    try {
      console.log('[SAVED] 📨 Loading saved messages...');
      const response = await apiClient.get<Message[]>('/api/messages');
      console.log('[SAVED] ✅ Saved messages loaded:', response.data.length);
      setMessages(response.data);
    } catch (e) {
      console.error('[SAVED] ❌ Failed to load saved messages:', e);
      setError(e as Error);
      // Don't fall back to empty data silently - show error to user
      Alert.alert(
        'Connection Error',
        'Unable to load your saved messages. Please check your connection and try again.',
        [{ text: 'OK' }]
      );
    } finally {
      setLoading(false);
  ...
  // When entering summary mode, prefetch first couple of bodies
  useEffect(() => {
    if (summaryMode && filteredMessages.length > 0) {
      prefetchBodies([filteredMessages[0]?.id, filteredMessages[1]?.id]);
    }
  }, [summaryMode, filteredMessages, prefetchBodies]);
  ...
  // Ensure proper initial positioning when summary header height is available
  useEffect(() => {
    if (summaryMode && summaryHeaderHeight > 0 && filteredMessages.length > 0) {
      // Reset group rows visibility when entering summary mode
      setShowGroupRows(true);
      // Small delay to ensure FlatList is fully rendered
      setTimeout(() => {
        if (flatListRef.current) {
          const targetOffset = summaryHeaderHeight + 24; // Same as snapToOffsets[0]
  ...
  }, [summaryMode, summaryHeaderHeight, filteredMessages.length]);
  ...
  const onRefresh = async () => {
    try {
      ...
      await apiClient.post('/api/backfill');
  ...
  return (
    ...
    >
      {summaryMode ? (
        <>
          {/* Fixed GroupsRow overlay - positioned above FlatList */}
          {showGroupRows && (
          ...
          onMomentumScrollEnd={(e) => {
            ...
            const cardHeight = windowHeight - 200;
            const baseOffset = summaryHeaderHeight + 16;
            let index = 0;

            // Calculate snap point for current scroll position
            for (let i = 0; i < filteredMessages.length; i++) {
              const snapPoint = i === 0 ? baseOffset + 24 + 8 : (() => {
                let offset = baseOffset + 24 + 8; // Start with first card (adjusted)
                for (let j = 1; j <= i; j++) {
                  offset += cardHeight + 20; // Add card height + spacing (8 + 12)
                }
                return offset;
              })();

              if (scrollY >= snapPoint - (cardHeight / 2)) {
                index = i;
              } else {
                break;
              }
            ...
            const currentId = filteredMessages[index]?.id;
            const nextId = filteredMessages[index + 1]?.id;
            prefetchBodies([currentId, nextId]);
          }}
          onScroll={(e) => {
            // Hide group rows when scrolling to second email or beyond
            const scrollY = e.nativeEvent.contentOffset.y;
            const threshold = summaryHeaderHeight + 50; // Small threshold after first card
            setShowGroupRows(scrollY < threshold);
          }}
          scrollEventThrottle={16}
          renderItem={({ item, index }) => {
            const src = extractBestImageSrc(item.body_html);
            const initials = item.sender_name
              ...
              .join('')
              .toUpperCase();
            const bgColor = stringToColor(item.sender_name);

            return (
              <View style={[styles.summaryCard, { height: windowHeight - 200, margin: 8, marginTop: index === 0 ? 24 : 12 }]}>
                {/* Hero image/avatar with enhanced design */}
                <TouchableOpacity onPress={() => navigation.navigate('Detail', { messageId: item.id })}>
                  {(() => {
                    const senderAvatarUrl = getSenderAvatarUrl(item);

                    if (src) {
                      // Newsletter image as hero background
                      return (
                        <View style={styles.heroImageContainer}>
                          {/* Background newsletter image */}
                          <Image source={{ uri: src }} style={styles.summaryHeroImage} resizeMode="contain" />
                          {/* Three dots overlay at bottom right */}
                          <TouchableOpacity
                            onPress={() => {
                              setContextMenu({ id: item.id, visible: true });
                            }}
                            style={styles.heroDotsOverlay}
                          >
                            <MaterialCommunityIcons name="dots-vertical" size={16} color="#000000" />
                          </TouchableOpacity>
                        </View>
                      );
                    } else if (senderAvatarUrl) {
                      // Sender-focused design when no newsletter image
                      return (
                        <View style={styles.heroBrandedFallback}>
                          <Image source={{ uri: senderAvatarUrl }} style={styles.largeSenderAvatar} resizeMode="cover" />
                          <Text style={[styles.senderNameLarge, { color: themeColors.text }]}>{item.sender_name}</Text>
                          <Text style={[styles.senderDomain, { color: themeColors.text }]}>{getSenderEmail(item).split('@')[1] || ''}</Text>
                          {/* Three dots overlay at bottom right */}
                          <TouchableOpacity
                            onPress={() => {
                              setContextMenu({ id: item.id, visible: true });
                      ...
                      // Fallback with initials when no images available
                      return (
                        <View style={styles.heroBrandedFallback}>
                          <View style={[styles.largeSenderAvatar, { backgroundColor: bgColor }]}>
                            <Text style={styles.largeSenderAvatarText}>{initials}</Text>
                          </View>
                          <Text style={[styles.senderNameLarge, { color: themeColors.text }]}>{item.sender_name}</Text>
                          <Text style={[styles.senderDomain, { color: themeColors.text }]}>{getSenderEmail(item).split('@')[1] || ''}</Text>
                          {/* Three dots overlay at bottom right */}
                ...
                <View style={styles.summaryContentContainer}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 20 }}>
                    <Text style={[styles.summaryMeta, { color: themeColors.text }]}>
                      {formatDateOrdinal(item.received_at)}
                    </Text>
                  </View>

                  {/* Subject (tap to open) */}
                  <TouchableOpacity onPress={() => navigation.navigate('Detail', { messageId: item.id })}>
                    <Text style={[styles.summarySubject, { color: themeColors.text }]}>{item.subject}</Text>
                  </TouchableOpacity>

                  {/* Summary/content - tap opens detail */}
                  <TouchableOpacity onPress={() => navigation.navigate('Detail', { messageId: item.id })}>
                    <Text style={[styles.summaryBody, { color: themeColors.text }]} numberOfLines={7}>
                      {generateLocalSummary(item)}
                    </Text>
                  </TouchableOpacity>
                </View>

                {/* Sender info section - positioned at bottom of card */}
                <View style={styles.summarySenderSection}>
                  <View style={styles.summarySenderInfo}>
                    <View style={[styles.summaryAvatar, { backgroundColor: bgColor }]}>
                      <Text style={styles.summaryAvatarText}>{initials}</Text>
                    </View>
                    <View style={{ marginLeft: 12, flex: 1 }}>
                      <Text style={[styles.senderNameStyled, { color: themeColors.text }]}>
                        {item.sender_name}
                      </Text>
                      <Text style={[styles.senderEmailStyled, { color: themeColors.text }]}>
                        {getSenderEmail(item)}
                      </Text>
                    </View>
                    {/* Up arrow for second email onwards */}
                    {index > 0 && (
                      <TouchableOpacity
                        onPress={scrollToTop}
      ...
      <>
        ...
        <SectionList
          ref={listRef}
          sections={sections}
          contentContainerStyle={!summaryMode ? { paddingTop: summaryHeaderHeight } : undefined}
          ListHeaderComponent={
        ...
        ListEmptyComponent={
          <View style={[styles.centerContainer, { backgroundColor: themeColors.background }]}>
            <Text style={{ color: themeColors.text }}>Your saved items will appear here.</Text>
            <Text style={{ color: themeColors.text, marginTop: 8, textAlign: 'center' }}>
              Save newsletters from your inbox to keep them organized.
            </Text>
          </View>
        }
        renderItem={({ item }) => (
      ...
      {/* Context menu modal for summary mode */}
      <Modal
        visible={summaryMode && contextMenu.visible}
        transparent
        animationType="fade"
        onRequestClose={() => setContextMenu({ id: null, visible: false })}
      >
        <TouchableOpacity
          style={styles.menuBackdrop}
          activeOpacity={1}
          onPress={() => setContextMenu({ id: null, visible: false })}
        >
          <View style={styles.menuContainer}>
            {/* Search removed per request; keep only read/save/delete */}
            <TouchableOpacity
              style={styles.menuItem}
              onPress={() => {
                const id = contextMenu.id;
                if (id != null) {
                  setMessages(current => current.map(m => m.id === id ? { ...m, is_read: !m.is_read } : m));
                }
                setContextMenu({ id: null, visible: false });
              }}
            >
              <Ionicons
                name={contextMenu.id ? (messages.find(m => m.id === contextMenu.id)?.is_read ? 'mail-unread' : 'mail') : 'mail'}
                size={18}
                color="#111827"
                style={{ marginRight: 8 }}
              />
              <Text style={styles.menuText}>
                {contextMenu.id ? (messages.find(m => m.id === contextMenu.id)?.is_read ? 'Mark as unread' : 'Mark as read') : 'Mark as read'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.menuItem}
              onPress={() => {
                const id = contextMenu.id;
                if (id != null) {
                  setMessages(current => current.map(m => m.id === id ? { ...m, is_saved: !m.is_saved } : m));
                }
                setContextMenu({ id: null, visible: false });
              }}
            >
              <Ionicons
                name={contextMenu.id ? (messages.find(m => m.id === contextMenu.id)?.is_saved ? 'bookmark' : 'bookmark-outline') : 'bookmark-outline'}
                size={18}
                color="#111827"
                style={{ marginRight: 8 }}
              />
      ...
      >
        <View style={[styles.searchOverlayBackdrop, { backgroundColor: themeColors.background }]} {...overlayPanResponder.panHandlers}>
          ...
          <FlatList
            data={searchOverlayQuery.trim() ? savedMessages.filter(m =>
